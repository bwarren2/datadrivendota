...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if email is None:
            raise Exception(
                "I need to have an email to send to. (Use --email)"
            )

        if privilege_level.lower() == 'look':
            pcode = PermissionCode.objects.create(
                upgrade_type=PermissionCode.LOOK
            )
        elif privilege_level.lower() == 'touch':
            pcode = PermissionCode.objects.create(
                upgrade_type=PermissionCode.TOUCH
            )
        else:
            raise Exception(
                "--level must be 'look' or 'touch', not {0}".format(
                    privilege_level
                )
            )
