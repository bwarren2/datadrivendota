...
def generateChart(
    ...
    # dubbed "base_".  A number inclusive of gains from stat growth is
    # prefixless. Modifiers (from stat points or items, w/e) will go into a
    # "modified_" variable, and final values (for plotting) are "final_")
    # NOTE: this convention is wrong with the numbers being imported as level
    # 1 right now.
...
def lineupChart(heroes, stat, level, width=800, height=500):
    ...
    x_vals = [
        key for key in
        sorted(hero_value, key=hero_value.get, reverse=True)
    ]
    y_vals = [
        hero_value[key] for key in
        sorted(hero_value, key=hero_value.get, reverse=True)
    ]
    col_vec = [
        # @todo: OK, fine, one acceptable place for postposed conditionals.
        # --kit 2014-02-16
        'red' if name in selected_names else 'green' for name in x_vals
    ]
    ...
    robjects.r(
        "df$name = factor(df$name, levels = unique(df$name), ordered=T)"
    )
...
def HeroPerformanceChart(
    ...
    # Ignore <10 min games
    matches = matches.filter(match__validity=Match.LEGIT)
...
def HeroSkillLevelBwChart(
    ...
    match_builds = SkillBuild.objects.filter(
        player_match_summary__hero__steam_id=hero,
        player_match_summary__in=pms_pool,
        level__in=levels
    ).select_related()
    for build in match_builds:
        build.skill_level = build.player_match_summary.match.skill

    if player is not None:
        player_pool = pms.filter(player__steam_id=player).select_related()
        player_builds = SkillBuild.objects.filter(
            player_match_summary__hero__steam_id=hero,
            player_match_summary__in=player_pool,
            level__in=levels
        ).select_related()
        for build in player_builds:
            build.skill_level = 'Player'
